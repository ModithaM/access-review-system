...
interface DashboardHeaderProps {
  ...
  onTrendChartTypeChange: (value: 'line' | 'area') => void;
}
...
export default function DashboardHeader({
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900 dark:shadow-none lg:p-8">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(56,189,248,0.18),_transparent_30%),radial-gradient(circle_at_top_left,_rgba(255,0,128,0.14),_transparent_30%)]" />
      <div className="relative flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
        <div className="max-w-2xl">
          <p className="mb-3 inline-flex items-center gap-2 rounded-full border border-gray-200 bg-white/90 px-3 py-1 text-xs font-medium text-gray-600 backdrop-blur dark:border-gray-700 dark:bg-gray-800/80 dark:text-gray-300">
          ...
          <p className="mt-3 text-sm text-gray-600 dark:text-gray-400 lg:text-base">
            Monitor platform performance and accessibility insights
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center lg:justify-end">
          ...
          <label className="flex items-center gap-2 rounded-2xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-600 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:shadow-none">
            ...
            <select
              ...
              className="bg-transparent text-sm text-gray-600 outline-none dark:text-gray-300"
              aria-label="Filter dashboard by public space category"
            >
          ...
          <div
            className="inline-flex rounded-2xl border border-gray-200 bg-white p-1 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:shadow-none"
            role="tablist"
            aria-label="Toggle growth chart type"
          >
            {(['line', 'area'] as const).map((option) => (
              <button
                key={option}
                type="button"
                role="tab"
                aria-selected={trendChartType === option}
                onClick={() => onTrendChartTypeChange(option)}
                className={`rounded-xl px-3 py-2 text-sm font-medium capitalize transition-all duration-300 ${
                  trendChartType === option
