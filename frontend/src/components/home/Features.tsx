...
const featureCards = [
  {
      ...
      <div className="flex flex-col lg:flex-row gap-8 h-full items-center">
        ...
        <div className="flex-1 flex flex-col justify-center">
          <div className="flex items-center gap-4 mb-6">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ backgroundColor: "#FF008018" }}>
              <Map size={26} style={{ color: "#FF0080" }} />
            </div>
            <div>
              <p className="text-[10px] uppercase font-bold tracking-widest text-gray-400">Live Discovery</p>
              <h3 className="text-2xl font-black tracking-tight text-gray-900">Interactive Map View</h3>
            </div>
          </div>
          ...
          <div className="flex gap-3 flex-wrap">
            {[
              ...
              <div
                key={loc.name}
                className="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-2xl px-4 py-2.5"
              >
                <div className="w-7 h-7 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: `${loc.color}18` }}>
                  <MapPinned size={13} style={{ color: loc.color }} />
                </div>
                <div>
                  <p className="text-xs font-bold text-gray-900 leading-none mb-1">{loc.name}</p>
                  <Stars rating={loc.rating} />
                </div>
                <span className="ml-1 text-[9px] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: `${loc.color}18`, color: loc.color }}>
                  {loc.badge}
                </span>
              </div>
            ))}
        ...
        <div className="flex-1 w-full relative">
          ...
          >
            <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: "#FF008022" }}>
    ...
    itemClassName: "bg-white border border-gray-100 shadow-md",
  },
  {
    id: "accessibility",
    content: (
      <div className="flex flex-col lg:flex-row gap-8 h-full items-center">
        <div className="flex-1 flex flex-col justify-center">
          <div className="w-14 h-14 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center mb-7">
            <Accessibility size={26} className="text-white" />
          </div>
          <h3 className="text-3xl font-black tracking-tight text-white mb-4 leading-tight">
          ...
          <div className="flex flex-wrap gap-2">
            {["♿ Ramp", "🛗 Lift", "👁 Braille", "🔇 Quiet", "🚪 Entry", "🔊 Audio"].map((chip) => (
              <span key={chip} className="text-xs font-bold bg-white/15 backdrop-blur-sm border border-white/20 text-white px-3 py-1.5 rounded-full">
                {chip}
              </span>
            ))}
          </div>
        ...
        <div className="flex-1 grid grid-cols-2 gap-4">
          {[
            ...
            <div key={item.label} className="bg-white/10 backdrop-blur-sm border border-white/15 rounded-3xl p-5 flex flex-col gap-2">
              <span className="text-2xl">{item.icon}</span>
              <span className="text-2xl font-black text-white">{item.count}</span>
              <span className="text-xs text-white/60 font-semibold">{item.label}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  },
  {
    ...
    content: (
      <div className="flex flex-col lg:flex-row gap-8 h-full items-center">
        <div className="flex-1 flex flex-col justify-center">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-7" style={{ backgroundColor: "#38BDF818" }}>
            <ShieldCheck size={26} style={{ color: "#38BDF8" }} />
          </div>
          <h3 className="text-3xl font-black tracking-tight text-gray-900 mb-4 leading-tight">
            Trusted & Inclusive Platform
          </h3>
          <p className="text-gray-500 text-base leading-relaxed">
            Community-powered and built on transparency — giving everyone reliable
            information to move freely through the world.
          </p>
        </div>
        <div className="flex-1 grid grid-cols-3 gap-4">
          {[
            ...
            <div key={label} className="flex flex-col items-center justify-center text-center rounded-3xl py-8 px-4" style={{ backgroundColor: `${color}0D` }}>
              <span className="text-4xl font-black leading-none" style={{ color }}>{val}</span>
  ...
  },
  {
    ...
    content: (
      <div className="flex flex-col lg:flex-row gap-8 h-full items-center">
        <div className="flex-1 flex flex-col justify-center">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-7" style={{ backgroundColor: "#FF008018" }}>
            <MapPinned size={26} style={{ color: "#FF0080" }} />
          </div>
          <h3 className="text-3xl font-black tracking-tight text-gray-900 mb-4 leading-tight">
            Discover Public Spaces
          </h3>
          <p className="text-gray-500 text-base leading-relaxed">
            Find parks, malls, transit hubs, and buildings with community-verified
            accessibility details. Never second-guess a venue again.
          </p>
        </div>
        <div className="flex-1 flex flex-col gap-3">
          {[
            { label: "Parks & Recreation", places: "68 places", color: "#FF0080" },
            { label: "Shopping Centres", places: "45 places", color: "#7928CA" },
            { label: "Transit & Transport", places: "87 places", color: "#0070F3" },
            { label: "Public Buildings", places: "52 places", color: "#38BDF8" },
          ].map((item) => (
            <div key={item.label} className="flex items-center justify-between bg-gray-50 border border-gray-100 rounded-2xl px-5 py-3.5">
              <div className="flex items-center gap-3">
                <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: item.color }} />
                <span className="text-sm font-semibold text-gray-800">{item.label}</span>
              </div>
              <span className="text-xs font-bold px-3 py-1 rounded-full" style={{ backgroundColor: `${item.color}18`, color: item.color }}>
                {item.places}
              </span>
            </div>
          ))}
  ...
  },
  {
    ...
    content: (
      <div className="flex flex-col lg:flex-row gap-8 h-full items-center">
        <div className="flex-1 flex flex-col justify-center">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-7" style={{ backgroundColor: "#7928CA18" }}>
            <Star size={26} style={{ color: "#7928CA" }} />
          </div>
          <h3 className="text-3xl font-black tracking-tight text-gray-900 mb-4 leading-tight">
            Share Accessibility Reviews
          </h3>
          <p className="text-gray-500 text-base leading-relaxed">
            Submit honest, experience-based reviews that help others navigate with
            confidence. Your voice makes every space more accessible.
          </p>
        </div>
        <div className="flex-1 flex flex-col gap-3">
          {[
            { name: "Ayesha M.", review: "Wheelchair ramp was easy to use!", stars: 5, tag: "Wheelchair ✓" },
            { name: "Ravi K.", review: "Elevator worked perfectly, great signage.", stars: 4, tag: "Elevator ✓" },
            { name: "Sophie L.", review: "Quiet area helped me focus — brilliant.", stars: 5, tag: "Quiet Zone ✓" },
          ].map((r) => (
            <div key={r.name} className="bg-gray-50 border border-gray-100 rounded-2xl px-5 py-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#7928CA] to-[#0070F3] flex items-center justify-center text-white text-xs font-black">
                    {r.name[0]}
                  </div>
              ...
              <p className="text-xs text-gray-500 leading-relaxed mb-2">"{r.review}"</p>
              <span className="text-[9px] font-bold px-2 py-0.5 rounded-full text-[#7928CA]" style={{ backgroundColor: "#7928CA18" }}>
                {r.tag}
              </span>
            </div>
          ))}
  ...
  },
  {
    ...
    content: (
      <div className="flex flex-col lg:flex-row gap-8 h-full items-center">
        <div className="flex-1 flex flex-col justify-center">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-7" style={{ backgroundColor: "#0070F318" }}>
            <UserRound size={26} style={{ color: "#0070F3" }} />
          </div>
          <h3 className="text-3xl font-black tracking-tight text-gray-900 mb-4 leading-tight">
            Personalised Profiles
          </h3>
          <p className="text-gray-500 text-base leading-relaxed">
            Track your contributions, manage reviews, and bookmark favourite accessible
            places. Build your accessibility journey in one place.
          </p>
        </div>
        <div className="flex-1 bg-gray-50 border border-gray-100 rounded-3xl p-6">
          <div className="flex items-center gap-4 mb-6 pb-5 border-b border-gray-100">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#0070F3] to-[#7928CA] flex items-center justify-center text-white text-xl font-black">
              A
            </div>
            <div>
              <p className="font-black text-gray-900">Alex Johnson</p>
              <p className="text-xs text-gray-400">Accessibility Advocate</p>
            </div>
            <span className="ml-auto text-[9px] font-bold px-3 py-1 rounded-full text-[#0070F3]" style={{ backgroundColor: "#0070F318" }}>
              Verified ✓
            </span>
          </div>
          <div className="grid grid-cols-3 gap-3 mb-5">
            {[
              ...
              <div key={label} className="flex flex-col items-center rounded-2xl py-3" style={{ backgroundColor: `${color}0D` }}>
                <span className="text-xl font-black" style={{ color }}>{val}</span>
                <span className="text-[9px] text-gray-400 font-bold uppercase tracking-wider mt-0.5">{label}</span>
              </div>
            ))}
          </div>
          <div className="text-xs text-gray-400 font-semibold mb-2">Recent Activity</div>
          {["Reviewed City Mall", "Bookmarked Central Park", "Added Metro Hub lift info"].map((a) => (
            <div key={a} className="flex items-center gap-2 py-2 border-b border-gray-100 last:border-0">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 pb-32">
        ...
        >
          {featureCards.map((card) => (
            <ScrollStackItem
              key={card.id}
              itemClassName={`min-h-[500px] ${card.itemClassName ?? ""}`}
              style={card.style}
            >
              {card.content}
            </ScrollStackItem>
